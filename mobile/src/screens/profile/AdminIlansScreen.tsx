...
type Props = NativeStackScreenProps<AppStackParamList, 'AdminIlans'>;
...
export function AdminIlansScreen({ navigation, route }: Props) {
  ...
  const [selectedDayCount, setSelectedDayCount] = useState<number | null>(7);
  const [isLoading, setIsLoading] = useState(true);
  const [isRefreshing, setIsRefreshing] = useState(false);
  const [isFilterLoading, setIsFilterLoading] = useState(false);
  const [errorMessage, setErrorMessage] = useState('');

  const filterByNeighborhood = (items: AdItem[]) =>
    items.filter((item) => item.sahibi?.mahalle?.id === neighborhoodId);
  ...
  const loadAds = async (refresh = false) => {
    try {
      ...
      setAds(filterByNeighborhood(adsData));
      setCategories(
  ...
  }, [searchText, selectedCategoryId, minPrice, maxPrice, selectedDayCount, isFocused, neighborhoodId]);
  ...
  return (
    <View style={styles.container}>
      <FlatList
        ...
        refreshControl={<RefreshControl refreshing={isRefreshing} onRefresh={() => void loadAds(true)} />}
        ListHeaderComponent={(
          <View style={styles.headerContent}>
            ...
            {errorMessage ? (
              <View style={styles.errorBox}>
                ...
                <Pressable style={styles.retryButton} onPress={() => void loadAds(false)}>
        ...
        ListEmptyComponent={<Text style={styles.emptyText}>Bu bölgede filtreye uygun ilan bulunamadı.</Text>}
      />
    </View>
  );
}
